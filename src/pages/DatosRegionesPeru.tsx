...
function DatosRegionesPeru() {
  const [datosRegiones, setDatosRegiones] = useState([
    { Country: "", Provinces: {} },
  ]);
  ...
  function restarFecha(fecha: any) {
    if (
      datosRegiones[0].Provinces[indexDatosRegiones][
  ...
  const DatosDepartamento = () => {
    try {
      let datoDepartamentoPeru =
        datosRegiones[0].Provinces[indexDatosRegiones][fecha()];
      if (typeof datoDepartamentoPeru !== "number") {
        console.error(
          "Parece que devuelve otro tipo de dato diferente a number"
        );
              ...
              >
                {loadingRegiones ? (
                  ...
                  datosRegiones.map((datoRegion) =>
                    Object.keys(datoRegion.Provinces).map(
                      (provincia, index) => (
